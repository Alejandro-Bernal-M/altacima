...
export default function Footer() {
  ...
  // Se obtiene la lista de enlaces desde i18n
  const rawLinks = t.raw('navLinks') as Array<{ href: string; label: string }>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full max-w-3xl max-h-[85vh] bg-[#131B24] border border-red-500/30 rounded-3xl p-6 sm:p-10 shadow-2xl flex flex-col text-gray-300">
            ...
            <div className="flex items-center justify-between pb-6 border-b border-white/10">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight">
                  {activeModal === 'privacy' ? t('modal.privacyTitle') : t('modal.termsTitle')}
                </h3>
              </div>
            ...
            <div className="overflow-y-auto pr-2 my-6 space-y-5 text-sm leading-relaxed text-gray-300 custom-scrollbar">
              {activeModal === 'privacy' ? (
                <>
                  <p>{t('privacyModal.intro')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('privacyModal.section1Title')}</h4>
                  <p>{t('privacyModal.section1Desc')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('privacyModal.section2Title')}</h4>
                  <p>{t('privacyModal.section2Desc')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('privacyModal.section3Title')}</h4>
                  <p>{t('privacyModal.section3Desc')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('privacyModal.section4Title')}</h4>
                  <p>{t('privacyModal.section4Desc')}</p>
                </>
              ) : (
                <>
                  <p>{t('termsModal.intro')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('termsModal.section1Title')}</h4>
                  <p>{t('termsModal.section1Desc')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('termsModal.section2Title')}</h4>
                  <p>{t('termsModal.section2Desc')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('termsModal.section3Title')}</h4>
                  <p>{t('termsModal.section3Desc')}</p>
                  <h4 className="text-white font-bold text-base pt-2">{t('termsModal.section4Title')}</h4>
                  <p>{t('termsModal.section4Desc')}</p>
                </>
              )}
            </div>
